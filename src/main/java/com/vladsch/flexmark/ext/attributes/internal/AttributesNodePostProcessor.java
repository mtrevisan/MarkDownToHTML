...
public class AttributesNodePostProcessor extends NodePostProcessor{
	...
	public Node getAttributeOwner(final NodeTracker state, final AttributesNode attributesNode){
		Node previous = attributesNode.getPreviousAnyNot(BlankLine.class, DoNotAttributeDecorate.class);
		Node attributeOwner;
		final Node parent = attributesNode.getParent();
		...
		if(previous == null){
			...
			if(parent instanceof Paragraph){
				if(parent.getParent() instanceof ParagraphItemContainer){
					final Node parentPreviousNotBlankLine = parent.getPreviousAnyNot(BlankLine.class);
			...
			if((!myOptions.assignTextAttributes && (previous instanceof Text || previous instanceof TextBase)) || previous.getEndOffset() < attributesNode.getStartOffset()){
				...
				if(myOptions.wrapNonAttributeText){
					...
					while(first instanceof Text || first instanceof DoNotAttributeDecorate){
					...
					if(hadDoNotDecorate){
						...
						final TextBase textBase = new TextBase();
	...
	static Node matchDelimitedSpans(final NodeTracker state, final AttributesNode attributesNode, Node previous){
		Node first = attributesNode.getPrevious();
		Node lastNonAttributesNode;
		final ArrayList<Node> unmatchedAttributes = new ArrayList<>();
		...
		if(!unmatchedAttributes.isEmpty()){
			...
			final Node previousNext = previous.getNext();
	...
	static void textBaseWrap(final NodeTracker state, Node lastNonAttributesNode, final Node lastNode, final TextBase textBase){
		while(lastNonAttributesNode != lastNode){
			final Node nextNode = lastNonAttributesNode.getNext();
	...
	public void process(final NodeTracker state, final Node node){
		if(node instanceof AttributesNode){
			final AttributesNode attributesNode = (AttributesNode)node;
			...
			final Node attributeOwner = getAttributeOwner(state, attributesNode);
			if(attributeOwner != null){
				...
				if(attributeOwner instanceof AnchorRefTarget)
					for(final Node attributeNode : attributesNode.getReversedChildren())
						if(attributeNode instanceof AttributeNode && ((AttributeNode)attributeNode).isId()){
		...
		if(node instanceof FencedCodeBlock fencedCodeBlock && myOptions.fencedCodeInfoAttributes){
			// see if has { after the first word
			final BasedSequence info = fencedCodeBlock.getInfo();
			final BasedSequence language = fencedCodeBlock.getInfoDelimitedByAny(CharPredicate.SPACE_TAB);
			final BasedSequence infoTail = info.subSequence(language.length()).trimStart();
			...
			if(pos >= 0){
				...
				final AttributesNode dummyBlock = new AttributesNode();
				myLightInlineParser.setBlock(dummyBlock); // dummy to hold possibly parsed attributes

				while(true){
					final int startIndex = myLightInlineParser.getIndex();
					final boolean parsed = myParserExtension.parse(myLightInlineParser);

					myLightInlineParser.spnl();
					final int index = myLightInlineParser.getIndex() + (myLightInlineParser.getIndex() == startIndex? 1: 0);
				...
				if(dummyBlock.hasChildren()){
					...
					final Node firstAttributes = dummyBlock.getFirstChild();
					final Node lastAttributes = dummyBlock.getLastChild();
					...
					for(final Node attributesNode : dummyBlock.getChildren()){
